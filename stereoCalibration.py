...
img_ptsR = []
obj_pts = []
images = glob.glob('./data/stereoL/*.jpg')


for i in range(14):
	imgL = cv2.imread(pathL+"imageL%d.jpg"%i)
	imgR = cv2.imread(pathR+"imageR%d.jpg"%i)
	imgL_gray = cv2.imread(pathL+"imageL%d.jpg"%i,0)
	imgR_gray = cv2.imread(pathR+"imageR%d.jpg"%i,0)
	...
	if retR and retL:
		obj_pts.append(objp)
		cv2.cornerSubPix(imgR_gray,cornersR,(13,17),(-1,-1),criteria)
		cv2.cornerSubPix(imgL_gray,cornersL,(13,17),(-1,-1),criteria)
